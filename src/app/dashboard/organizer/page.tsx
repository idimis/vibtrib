...
const OrganizerDashboard: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [responseEvents, responseProfile] = await Promise.all([
        fetch("http://vibetribe-backend-shj1ro-029a2b-38-45-65-22.traefik.me/api/v1/events/organizer", {
          method: "GET",
  ...
  const fetchVouchers = async () => {
    ...
    try {
      const response = await fetch("http://vibetribe-backend-shj1ro-029a2b-38-45-65-22.traefik.meapi/v1/vouchers/upcoming", {
        method: "GET",
        headers: { Authorization: `Bearer ${token}` },
      });
